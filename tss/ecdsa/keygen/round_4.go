...
func (round *round4) Start() ([]tss.ParsedMessage, interface{}, *tss.Error) {
	if round.started {
		return nil, nil, round.WrapError(errors.New("round already started"))
	}
	round.number = 4
	round.started = true
	round.resetOK()
	...
	if len(culprits) > 0 {
		return nil, nil, round.WrapError(errors.New("paillier verify failed"), culprits...)
	}

	return nil, round.save, nil
}
