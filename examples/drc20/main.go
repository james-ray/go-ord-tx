...
// var BTCNETPARAM = chaincfg.RegressionNetParams

const (
	DogeMinVal      = 100000      //doge utxo 最小val
	DogeDeployFee   = 10000000000 //部署 100个Doge 币
	DogeMintFee     = 50000000    //铸币 0.5 个 Doge 币
...
func (c *drc20) SignRawTransaction(signIn *SignInput) (*SignResult, error) {
	...
	if signIn.Type == "drc20" {
		...
		builder.AddOp(txscript.OP_1).AddData(a.SerializePubKey()).AddOp(txscript.OP_1)
		...
		addr_new, err := btcutil.NewAddressScriptHash(redeemScript, &DOGEParams)
		if err != nil {
			return nil, err
		}
	...
	rawTx := ""
	//2.向这个地址打钱的交易
	centerAddress1, err := btcutil.DecodeAddress(centerAddress, &DOGEParams)
	if err != nil {
		return nil, err
	}
	mtxOne := wire.NewMsgTx(1)
	...
	srcAddr, err := btcutil.DecodeAddress(signIn.SrcAddr, &DOGEParams)
	if err != nil {
		return nil, err
	}
	...
	destAddr, err := btcutil.DecodeAddress(signIn.DestAddr, &DOGEParams)
	if err != nil {
		return nil, err
	}
	...
	for i, input := range vouts {
		...
		// 获取vin的script的类型
		scriptClass, _, _, err := txscript.ExtractPkScriptAddrs(txInPkScript, &DOGEParams)
	...
	payToAddressAddr, err := btcutil.DecodeAddress(payToAddressStr, &DOGEParams)
	...
	prevOut1 := wire.NewOutPoint(&needSpendHash, 0)
	txIn1 := wire.NewTxIn(prevOut1, []byte{}, [][]byte{})
	mtxTwo.AddTxIn(txIn1)
	sig, err := txscript.RawTxInSignature(mtxTwo, 0, redeemScript, txscript.SigHashAll, priv)
	if err != nil {
		return nil, err
	}
...
func (c *drc20) GetWalletAccountFromWif() (*WalletAccount, error) {
	...
	if len(wif) > 0 {
		...
		isDoge := btcWif.IsForNet(&DOGEParams)
		if isDoge == false {
			return nil, errors.New("key type error")
		}
		pk := btcWif.SerializePubKey()
		fmt.Println("pk : ", hex.EncodeToString(pk))
		address, err := btcutil.NewAddressPubKeyHash(btcutil.Hash160(pk), &DOGEParams)
		if err != nil {
			fmt.Println("Wif err : ", err.Error())
			return nil, err
...
func main() {
	initParams()
	netParams := &DOGEParams
	btcApiClient := mempool.NewDOGEClient()
	...
	unspentList, err := btcApiClient.ListUnspentForDoge(pay2pubkeyHashAddress)
	...
	signInput := &SignInput{
		Coin:       "drc20",
		Symbol:     "drc20",
		PrivateKey: "QNykotPn5HQs8GkWP1ZrnP3uZornUbL4eQFB64FrmEvxhdiNp8h4",
		SrcAddr:    pay2pubkeyHashAddress.String(),
		DestAddr:   "DFqr5mciNKn1coAT4KMy8EQTDpedVhHujo", //pay2pubkeyHashAddress.String()
		Fee:        20000000,
		//SecondFee:  20000000,
		Amount:     totalAmount,
		Change:     0,
