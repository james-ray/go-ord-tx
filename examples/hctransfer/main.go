...
func main() {
	netParams := &hcchaincfg.MainNetParams
	...
	//taprootAddress, err := hcutil.NewAddressTaproot(schnorr.SerializePubKey(txscript.ComputeTaprootKeyNoScript(privateKey.PubKey())), netParams)
	pubKeyHash := hcutil.Hash160(privateKey.PubKey().SerializeCompressed())
	addressPubkeyHash, err := hcutil.NewAddressPubKeyHash(pubKeyHash, netParams, 0)
	if err != nil {
		log.Fatal(err)
	}
	...
	unspentList, err := btcApiClient.ListUnspentForHc(addressPubkeyHash)
